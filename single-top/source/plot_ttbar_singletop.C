...
// MACRO PRINCIPAL
// =====================================================================
void plot_ttbar_singletop()                                            //função void é a função principal que será executada quando rodar o código,os arquivos
{
    // ================================================================
    ...
    const double sigmattbar_tW = 3.0466e4; // pb tW 2354+ ttbar NLO 28112                               // Seção de choque total para o processo pp -> tW no nível de Leading Order (LO) em pb       
    ...
    hEta_ttbar_tW->SetMaximum(hEta_ttbar_tW->GetMaximum() * 500);
   ...
   // ================================================================= 
    ...
    if (hPt_ttbar_tW) {                                                   //Verifica se o histograma de pt para o processo pp -> ttbar (LO) existe
        ...
        hPt_ttbar_tW->GetXaxis()->SetRangeUser(800, 3500);    
        hPt_ttbar_tW->SetMaximum(hPt_NLO->GetMaximum() * 100);             //Define o intervalo do eixo X do histograma de pt para o processo pp -> ttbar (LO) de 0 a 3500 GeV
        hPt_ttbar_tW->SetTitle(";p_{T} [GeV];d#sigma/dp_{T} [pb/GeV]");  //Titulo do histograma de pt para o processo pp -> ttbar (LO)
        hPt_ttbar_tW->GetXaxis()->CenterTitle(true);                     
        hPt_ttbar_tW->GetYaxis()->CenterTitle(true);                  
        hPt_ttbar_tW->GetYaxis()->SetTitleOffset(1.7);                   //Define o deslocamento do título do eixo Y do histograma de pt para o processo pp -> ttbar (LO)

        hPt_ttbar_tW->DrawCopy("HIST");                                  //Desenha o histograma de pt para o processo pp -> ttbar (LO) usando Draw para garantir que as configurações de estilo sejam aplicadas corretamente
    }
