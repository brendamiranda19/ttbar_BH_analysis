...
void plot_ttbar_overlay()                                                //função void é a função principal que será executada quando rodar o código, e é onde todo o código de leitura dos arquivos, manipulação dos histogramas e plotagem dos gráficos será escrito
{
    ...
    hEta_LO_p->SetLineColor(kBlack);                                   //Define a cor da linha do histograma de eta para o processo pp -> ttbar (LO) como preto (kBlack)
    hEta_LO_p->SetLineWidth(4);                                        //Define a largura da linha do histograma de eta para o processo pp -> ttbar (LO) como 4, para deixar a linha bem visível

    hEta_NLO_p->SetLineColor(kGray+1);                                 //Define a cor da linha do histograma de eta para o processo pp -> ttbar (NLO) como cinza claro (kGray+1)
    hEta_NLO_p->SetLineStyle(2);                                       //Define o estilo da linha do histograma de eta para o processo pp -> ttbar (NLO) como 2, que é uma linha tracejada
    hEta_NLO_p->SetLineWidth(4);                                       //Define a largura da linha do histograma de eta para o processo pp -> ttbar (NLO) como 4, para deixar a linha bem visível

    hEta_BH4_p->SetLineColor(kRed);                                    //Define a cor da linha do histograma de eta para o processo pp -> ttbar via BH com n=6, MD=4 TeV, MBH=8 TeV como vermelho (kRed)
    hEta_BH4_p->SetLineStyle(3);                                       //Define o estilo da linha do histograma de eta para o processo pp -> ttbar via BH com n=6, MD=4 TeV, MBH=8 TeV como 3, que é uma linha pontilhada
    hEta_BH4_p->SetLineWidth(4);                                       //Define a largura da linha do histograma de eta para o processo pp -> ttbar via BH com n=6, MD=4 TeV, MBH=8 TeV como 4, para deixar a linha bem visível

    hEta_BH6_p->SetLineColor(kGreen+2);                                //Define a cor da linha do histograma de eta para o processo pp -> ttbar via BH com n=6, MD=4 TeV, MBH=9 TeV como verde (kGreen+2)
    hEta_BH6_p->SetLineStyle(4);                                       //Define o estilo da linha do histograma de eta para o processo pp -> ttbar via BH com n=6, MD=4 TeV, MBH=9 TeV como 4, que é uma linha com traços e pontos
    hEta_BH6_p->SetLineWidth(4);                                       //Define a largura da linha do histograma de eta para o processo pp -> ttbar via BH com n=6, MD=4 TeV, MBH=9 TeV como 4, para deixar a linha bem visível

    hEta_BHX_p->SetLineColor(kBlue);                                   //Define a cor da linha do histograma de eta para o processo pp -> ttbar via BH com n=6, MD=4 TeV, MBH=10 TeV como azul (kBlue)
    hEta_BHX_p->SetLineStyle(5);                                       //Define o estilo da linha do histograma de eta para o processo pp -> ttbar via BH com n=6, MD=4 TeV, MBH=10 TeV como 5, que é uma linha com traços longos e pontos
    hEta_BHX_p->SetLineWidth(4);                                       //Define a largura da linha do histograma de eta para o processo pp -> ttbar via BH com n=6, MD=4 TeV, MBH=10 TeV como 4, para deixar a linha bem visível

    hEta_LO_p->SetTitle(";#eta;d#sigma/d#eta [pb]");                   //Define o título do histograma de eta para o processo pp -> ttbar (LO) como ";#eta;d#sigma/d#eta [pb]", onde o primeiro ";" indica que não há título para o eixo Y, "#eta" é o título do eixo X, e "d#sigma/d#eta [pb]" é o título do eixo Y, indicando que a distribuição é a seção de choque diferencial em função de eta em pb
    hEta_LO_p->GetXaxis()->CenterTitle(true);                          //Centraliza o título do eixo X do histograma de eta para o processo pp -> ttbar (LO), para deixar o título do eixo X bem posicionado
    hEta_LO_p->GetYaxis()->CenterTitle(true);                          //Centraliza o título do eixo Y do histograma de eta para o processo pp -> ttbar (LO), para deixar o título do eixo Y bem posicionado

    hEta_LO_p->Draw("HIST");                                           //Desenha o histograma de eta para o processo pp -> ttbar (LO) usando a opção "HIST", que desenha apenas a linha do histograma sem os erros
    hEta_NLO_p->Draw("HIST SAME");                                     //Desenha o histograma de eta para o processo pp -> ttbar (NLO) usando a opção "HIST SAME", que desenha apenas a linha do histograma e sobrepõe ao histograma anterior (LO)
    hEta_BH4_p->Draw("HIST SAME");                                     //Desenha o histograma de eta para o processo pp -> ttbar via BH com n=6, MD=4 TeV, MBH=8 TeV usando a opção "HIST SAME", que desenha apenas a linha do histograma e sobrepõe aos histogramas anteriores (LO e NLO)
    hEta_BH6_p->Draw("HIST SAME");                                     //Desenha o histograma de eta para o processo pp -> ttbar via BH com n=6, MD=4 TeV, MBH=9 TeV usando a opção "HIST SAME", que desenha apenas a linha do histograma e sobrepõe aos histogramas anteriores (LO, NLO e BH4)
    hEta_BHX_p->Draw("HIST SAME");                                     //Desenha o histograma de eta para o processo pp -> ttbar via BH com n=6, MD=4 TeV, MBH=10 TeV usando a opção "HIST SAME", que desenha apenas a linha do histograma e sobrepõe aos histogramas anteriores (LO, NLO, BH4 e BH6)
   ...
   // =================================================================
    ...
    if (hPt_LO_p) {
        hPt_LO_p->SetLineColor(kBlack);                                //Define a cor da linha do histograma de pt para o processo pp -> ttbar (LO) como preto (kBlack)
        hPt_LO_p->SetLineWidth(4);                                     //Define a largura da linha do histograma de pt para o processo pp -> ttbar (LO) como 4, para deixar a linha bem visível

        hPt_LO_p->GetXaxis()->SetRangeUser(0, 1200);                   //Define o intervalo do eixo X do histograma de pt para o processo pp -> ttbar (LO) de 0 a 1200 GeV, para focar na região de interesse do gráfico de pt
        hPt_LO_p->SetTitle(";p_{T} [GeV];d#sigma/dp_{T} [pb/GeV]");    //Define o título do histograma de pt para o processo pp -> ttbar (LO) como ";p_{T} [GeV];d#sigma/dp_{T} [pb/GeV]", onde o primeiro ";" indica que não há título para o eixo Y, "p_{T} [GeV]" é o título do eixo X, e "d#sigma/dp_{T} [pb/GeV]" é o título do eixo Y, indicando que a distribuição é a seção de choque diferencial em função de pt em pb/GeV
        hPt_LO_p->GetXaxis()->CenterTitle(true);                       //Centraliza o título do eixo X do histograma de pt para o processo pp -> ttbar (LO), para deixar o título do eixo X bem posicionado
        hPt_LO_p->GetYaxis()->CenterTitle(true);                       //Centraliza o título do eixo Y do histograma de pt para o processo pp -> ttbar (LO), para deixar o título do eixo Y bem posicionado
        hPt_LO_p->GetYaxis()->SetTitleOffset(1.7);                     //Define o deslocamento do título do eixo Y do histograma de pt para o processo pp -> ttbar (LO) como 1.7, para deixar o título do eixo Y mais afastado do eixo, já que o título do eixo Y é rotacionado e fica mais próximo do eixo

        hPt_LO_p->Draw("HIST");                                        //Desenha o histograma de pt para o processo pp -> ttbar (LO) usando a opção "HIST", que desenha apenas a linha do histograma sem os erros
    }

    if (hPt_NLO_p) {
        hPt_NLO_p->SetLineColor(kGray+1);                              //Define a cor da linha do histograma de pt para o processo pp -> ttbar (NLO) como cinza claro (kGray+1)
        hPt_NLO_p->SetLineWidth(4);                                    //Define a largura da linha do histograma de pt para o processo pp -> ttbar (NLO) como 4, para deixar a linha bem visível
        hPt_NLO_p->SetLineStyle(2);                                    //Define o estilo da linha do histograma de pt para o processo pp -> ttbar (NLO) como 2, que é uma linha tracejada
        hPt_NLO_p->Draw("HIST SAME");                                  //Desenha o histograma de pt para o processo pp -> ttbar (NLO) usando a opção "HIST SAME", que desenha apenas a linha do histograma e sobrepõe ao histograma anterior (LO)
    }

    if (hPt_BH4_p) { hPt_BH4_p->SetLineColor(kRed);     hPt_BH4_p->SetLineStyle(3); hPt_BH4_p->SetLineWidth(4); hPt_BH4_p->Draw("HIST SAME"); }       //Desenha o histograma de pt para o processo pp -> ttbar via BH com n=6, MD=4 TeV, MBH=8 TeV usando a opção "HIST SAME", que desenha apenas a linha do histograma e sobrepõe aos histogramas anteriores (LO e NLO)
    if (hPt_BH6_p) { hPt_BH6_p->SetLineColor(kGreen+2); hPt_BH6_p->SetLineStyle(4); hPt_BH6_p->SetLineWidth(4); hPt_BH6_p->Draw("HIST SAME"); }       //Desenha o histograma de pt para o processo pp -> ttbar via BH com n=6, MD=4 TeV, MBH=9 TeV usando a opção "HIST SAME", que desenha apenas a linha do histograma e sobrepõe aos histogramas anteriores (LO, NLO e BH4)
    if (hPt_BHX_p) { hPt_BHX_p->SetLineColor(kBlue);    hPt_BHX_p->SetLineStyle(5); hPt_BHX_p->SetLineWidth(4); hPt_BHX_p->Draw("HIST SAME"); }       //Desenha o histograma de pt para o processo pp -> ttbar via BH com n=6, MD=4 TeV, MBH=10 TeV usando a opção "HIST SAME", que desenha apenas a linha do histograma e sobrepõe aos histogramas anteriores (LO, NLO, BH4 e BH6)
   ...
   // =================================================================
    ...
    leg->AddEntry(hPt_LO_p,  "pp #rightarrow t#bar{t} LO (pQCD)",  "l");   //Legenda para o histograma de pt do processo pp -> ttbar (LO), com a descrição "pp #rightarrow t#bar{t} LO (pQCD)" e o estilo de linha "l" (linha)
    leg->AddEntry(hPt_NLO_p, "pp #rightarrow t#bar{t} NLO (pQCD)", "l");   //Legenda para o histograma de pt do processo pp -> ttbar (NLO), com a descrição "pp #rightarrow t#bar{t} NLO (pQCD)" e o estilo de linha "l" (linha)
    leg->AddEntry(hPt_BH4_p, "n=6, M_{D}=4 TeV, M_{BH}=8 TeV",    "l");    //Legenda para o histograma de pt do processo pp -> ttbar via BH com n=6, MD=4 TeV, MBH=8 TeV, com a descrição "n=6, M_{D}=4 TeV, M_{BH}=8 TeV" e o estilo de linha "l" (linha)
    leg->AddEntry(hPt_BH6_p, "n=6, M_{D}=4 TeV, M_{BH}=9 TeV",    "l");    //Legenda para o histograma de pt do processo pp -> ttbar via BH com n=6, MD=4 TeV, MBH=9 TeV, com a descrição "n=6, M_{D}=4 TeV, M_{BH}=9 TeV" e o estilo de linha "l" (linha)
    leg->AddEntry(hPt_BHX_p, "n=6, M_{D}=4 TeV, M_{BH}=10 TeV",   "l");    //Legenda para o histograma de pt do processo pp -> ttbar via BH com n=6, MD=4 TeV, MBH=10 TeV, com a descrição "n=6, M_{D}=4 TeV, M_{BH}=10 TeV" e o estilo de linha "l" (linha)
    ...
    if (hPt_LO_p) {                                                 //Verifica se o histograma de pt para o processo pp -> ttbar (LO) existe, para evitar erros caso o histograma não tenha sido obtido corretamente dos arquivos .root
        hPt_LO_p->SetLineColor(kBlack);                             //Define a cor da linha do histograma de pt para o processo pp -> ttbar (LO) como preto (kBlack)
        hPt_LO_p->SetLineWidth(4);                                  //Define a largura da linha do histograma de pt para o processo pp -> ttbar (LO) como 4, para deixar a linha bem visível

        hPt_LO_p->GetXaxis()->SetRangeUser(0, 3500);                //Define o intervalo do eixo X do histograma de pt para o processo pp -> ttbar (LO) de 0 a 3500 GeV, para mostrar melhor a região de alta pT onde os efeitos de BH podem ser mais evidentes, já que no gráfico anterior o eixo X foi limitado a 1200 GeV para focar na região de interesse, mas isso pode esconder as diferenças entre os modelos em pT mais altos, então criamos um gráfico separado para mostrar essa região
        hPt_LO_p->SetTitle(";p_{T} [GeV];d#sigma/dp_{T} [pb/GeV]"); //Titulo do histograma de pt para o processo pp -> ttbar (LO), com a descrição ";p_{T} [GeV];d#sigma/dp_{T} [pb/GeV]", onde o primeiro ";" indica que não há título para o eixo Y, "p_{T} [GeV]" é o título do eixo X, e "d#sigma/dp_{T} [pb/GeV]" é o título do eixo Y, indicando que a distribuição é a seção de choque diferencial em função de pt em pb/GeV
        hPt_LO_p->GetXaxis()->CenterTitle(true);                    //Centraliza o título do eixo X do histograma de pt para o processo pp -> ttbar (LO), para deixar o título do eixo X bem posicionado
        hPt_LO_p->GetYaxis()->CenterTitle(true);                    //Centraliza o título do eixo Y do histograma de pt para o processo pp -> ttbar (LO), para deixar o título do eixo Y bem posicionado
        hPt_LO_p->GetYaxis()->SetTitleOffset(1.7);                  //Define o deslocamento do título do eixo Y do histograma de pt para o processo pp -> ttbar (LO) como 1.7, para deixar o título do eixo Y mais afastado do eixo, já que o título do eixo Y é rotacionado e fica mais próximo do eixo

        hPt_LO_p->Draw("HIST");                                     //Desenha o histograma de pt para o processo pp -> ttbar (LO) usando a opção "HIST", que desenha apenas a linha do histograma sem os erros, para mostrar a distribuição de pt para o processo pp -> ttbar (LO) no gráfico separado para a região de alta pT
    }

    if (hPt_NLO_p) {                                                //Verifica se o histograma de pt para o processo pp -> ttbar (NLO) existe, para evitar erros caso o histograma não tenha sido obtido corretamente dos arquivos .root
        hPt_NLO_p->SetLineColor(kGray+1);                           //Define a cor da linha do histograma de pt para o processo pp -> ttbar (NLO) como cinza claro (kGray+1)
        hPt_NLO_p->SetLineWidth(4);                                 //Define a largura da linha do histograma de pt para o processo pp -> ttbar (NLO) como 4, para deixar a linha bem visível
        hPt_NLO_p->SetLineStyle(2);                                 //Define o estilo da linha do histograma de pt para o processo pp -> ttbar (NLO) como 2, que é uma linha tracejada, para diferenciar visualmente do histograma de pt para o processo pp -> ttbar (LO) e destacar a contribuição das correções de NLO
    ...
    legPt2->AddEntry(hPt_LO_p,  "pp #rightarrow t#bar{t} LO (pQCD)",  "l"); //Legenda para o histograma de pt do processo pp -> ttbar (LO), com a descrição "pp #rightarrow t#bar{t} LO (pQCD)" e o estilo de linha "l" (linha)
    legPt2->AddEntry(hPt_NLO_p, "pp #rightarrow t#bar{t} NLO (pQCD)", "l"); //Legenda para o histograma de pt do processo pp -> ttbar (NLO), com a descrição "pp #rightarrow t#bar{t} NLO (pQCD)" e o estilo de linha "l" (linha)
    legPt2->AddEntry(hPt_BH4_p, "n=6, M_{D}=4 TeV, M_{BH}=8 TeV",     "l"); //Legenda para o histograma de pt do processo pp -> ttbar via BH com n=6, MD=4 TeV, MBH=8 TeV, com a descrição "n=6, M_{D}=4 TeV, M_{BH}=8 TeV" e o estilo de linha "l" (linha)
    legPt2->AddEntry(hPt_BH6_p, "n=6, M_{D}=4 TeV, M_{BH}=9 TeV",     "l"); //Legenda para o histograma de pt do processo pp -> ttbar via BH com n=6, MD=4 TeV, MBH=9 TeV, com a descrição "n=6, M_{D}=4 TeV, M_{BH}=9 TeV" e o estilo de linha "l" (linha)
    legPt2->AddEntry(hPt_BHX_p, "n=6, M_{D}=4 TeV, M_{BH}=10 TeV",    "l"); //Legenda para o histograma de pt do processo pp -> ttbar via BH com n=6, MD=4 TeV, MBH=10 TeV, com a descrição "n=6, M_{D}=4 TeV, M_{BH}=10 TeV" e o estilo de linha "l" (linha)
