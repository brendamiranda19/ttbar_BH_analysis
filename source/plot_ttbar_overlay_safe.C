...
void plot_ttbar_overlay_safe()                                            //função void é a função principal que será executada quando rodar o código,os arquivos
{
    ...
    // arquivos
    TFile* ftW  = TFile::Open("/home/brenda_rolin/Documentos/programas/BlackMax-2.02.0/BlackMax/ttbar_BH_analysis/single-top/analysis_outputs/output_tW.root"); 
    ...
    c1->Update();
    c1->SaveAs("ttbar_tW_100TeV_5M_n6_md4_mbh_fig1.eps.eps");  
   ...
   // ================================================================= 
    ...
    //}
     ...
     if (hPt_ttbar_tW) {                                                  //Verifica se o histograma de pt para o processo pp -> ttbar (LO) existe
        hPt_ttbar_tW->SetLineColor(kBlack);                              //Define a cor da linha do histograma 
        hPt_ttbar_tW->SetLineWidth(4);                                   //Define a largura da linha do histograma de pt para o processo pp -> ttbar (LO) como 4, para deixar a linha bem visível

        hPt_ttbar_tW->GetXaxis()->SetRangeUser(0, 1200);                 //Define o intervalo do eixo X do histograma de pt para o processo pp -> ttbar (LO) de 0 a 1200 GeV
        hPt_ttbar_tW->SetTitle(";p_{T} [GeV];d#sigma/dp_{T} [pb/GeV]");  //Titulo do histograma de pt para o processo pp -> ttbar (LO)
        hPt_ttbar_tW->GetXaxis()->CenterTitle(true);                     
        hPt_ttbar_tW->GetYaxis()->CenterTitle(true);                  
        hPt_ttbar_tW->GetYaxis()->SetTitleOffset(1.7);                   //Define o deslocamento do título do eixo Y do histograma de pt para o processo pp -> ttbar (LO)
        //hPt_ttbar_tW->SetMaximum(hPt_NLO->GetMaximum() * 300);
        hPt_ttbar_tW->DrawCopy("HIST");                                      //Desenha o histograma de pt para o processo pp -> ttbar (LO)
    }

    if (hPt_BH4) { 
        hPt_BH4->Draw("HIST SAME");
        hPt_BH4->SetLineColor(kRed);                                    
        hPt_BH4->SetLineStyle(3);                                      
    ...
    legPt2->AddEntry(hPt_ttbar_tW, "pp #rightarrow t#bar{t}+tW (pQCD)", "l"); 
    ...
    legPt2->Draw();     
    c2->SaveAs("ttbar_tW_100TeV_5M_n6_md4_mbh_fig2.eps");            //Salva o canvas "c2" como um arquivo de imagem no formato EPS com o nome "ttbar_overlay_100TeV_5M_n6_md4_mbh.eps", para que a figura possa ser visualizada e compartilhada em formato de documento EPS, mantendo a qualidade vetorial da figura
    c2->SaveAs("ttbar_tW_100TeV_5M_n6_md4_mbh_fig2.png");            //Salva o canvas "c2" como um arquivo de imagem no formato PNG com o nome "ttbar_overlay_100TeV_5M_n6_md4_mbh.png", para que a figura possa ser visualizada e compartilhada em formato de imagem
    ...
    if (hPt_ttbar_tW) {                                                  //Verifica se o histograma de pt para o processo pp -> ttbar (LO) existe
        ...
        hPt_ttbar_tW->GetXaxis()->SetRangeUser(0, 3500);                 //Define o intervalo do eixo X do histograma de pt para o processo pp -> ttbar (LO) de 0 a 1200 GeV
        hPt_ttbar_tW->SetTitle(";p_{T} [GeV];d#sigma/dp_{T} [pb/GeV]");  //Titulo do histograma de pt para o processo pp -> ttbar (LO)
        hPt_ttbar_tW->GetXaxis()->CenterTitle(true);                     
        hPt_ttbar_tW->GetYaxis()->CenterTitle(true);                  
        hPt_ttbar_tW->GetYaxis()->SetTitleOffset(1.7);                   //Define o deslocamento do título do eixo Y do histograma de pt para o processo pp -> ttbar (LO)
    ...
    legPt3->AddEntry(hPt_ttbar_tW, "pp #rightarrow t#bar{t}+tW (pQCD)", "l"); 
